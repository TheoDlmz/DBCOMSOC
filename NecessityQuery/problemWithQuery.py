...
def scoreTuple(T,N,m,c):
    ...
    if lenc == 1:
        ...
        for i in range(lenc):
            ...
            ci = c.copy()
            ci.pop(i)
            scoreT,Fset,Fscore = scoreTuple(Ti,Ni,m,ci)
            if scoreT < scorei:
        ...
        if notInC:
            ...
            while j < (len(Fset)) and a:
                ...
                while k < (len(Fset[j])) and b:
                    if (Fset[j][k] == 1 and T[i][k] == 0) or (Fset[j][k] == -1 and T[i][j] == 1):
                        b = False
                    k+=1
...
def possibilityTuple(T,N,m,c):
    clen = len(c)
    score, Fset, Fscore = scoreTuple(T,N,m,c)
    g = mf.GraphInt()
...
def testMW(n,m,k,phi=0.3,psi=0.5):
    ...
    print("time : ",(tb-ta)/m)
    print("test 2")
    b = [[i,j] for i in range(m) for j in range(i)]
    ta = time.time()
    possibilityTupleSet(v,m,b)
    tb = time.time()
    ntest = m*(m+1)//2
    print("time : ",(tb-ta)/len(b))
